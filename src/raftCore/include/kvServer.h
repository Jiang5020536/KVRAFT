...
class KvServer : raftKVRpcProctoc::kvServerRpc {
 private:
  ...
  int m_maxRaftState;                               // snapshot if log grows this big		//创建快照的阈值

  // Your definitions here.
  std::string m_serializedKVData;  // todo ： 序列化后的kv数据，理论上可以不用，但是目前没有找到特别好的替代方法
  SkipList<std::string, std::string> m_skipList;
  std::unordered_map<std::string, std::string> m_kvDB;		//存储键值对的数据库

  std::unordered_map<int, LockQueue<Op> *> waitApplyCh;		//确保分布式一致的问题//一个映射，raft日志索引映射到操作等待队列      todo
  // index(raft) -> chan  //？？？字段含义   waitApplyCh是一个map，键是int，值是Op类型的管道

  std::unordered_map<std::string, int> m_lastRequestId;  // clientid -> requestID  检查请求是否重复

  // last SnapShot point , raftIndex
  int m_lastSnapShotRaftLogIndex;		//上一个快照点的raft日志索引，用于快照恢复和维护
 ...
 public:
   ...
   * @param message
   */
  void GetCommandFromRaft(ApplyMsg message);	//从raft节点获取并处理应用到状态机的命令

  bool ifRequestDuplicate(std::string ClientId, int RequestId);	//检测客户端请求是否重复
  ...
  ////一直等待raft传来的applyCh
  void ReadRaftApplyCommandLoop();//持续读取来自 Raft 节点的应用命令，并执行。
  ...
  std::string MakeSnapShot(); //创建并返回当前快照

 public:  // for rpc
  void PutAppend(google::protobuf::RpcController *controller, const ::raftKVRpcProctoc::PutAppendArgs *request,
                 ::raftKVRpcProctoc::PutAppendReply *response, ::google::protobuf::Closure *done) override;//实现 RPC 接口处理 PUT 或 APPEND 请求

  void Get(google::protobuf::RpcController *controller, const ::raftKVRpcProctoc::GetArgs *request,
           ::raftKVRpcProctoc::GetReply *response, ::google::protobuf::Closure *done) override; //实现 RPC 接口处理 GET 请求
 ...
 private:
  ...
  void serialize(Archive &ar, const unsigned int version)  //定义如何序列化和反序列化 KvServer 对象，包括快照数据和请求 ID 映射。
  {
